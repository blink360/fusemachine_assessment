...
function LeagueStandingsTable(props) {
  ...
  let searchFunction = (filter) => {
    if (filter) {
      const filteredClubNames = props.seasonData.filter((key) =>
        ...
        <Row>
          <Col xs={12} md={6}>
            ...
            <Form.Control
              as="select"
              onChange={(e) => {
                fetchDataForTheSeason(e.target.value);
              }}
            >
              {seasons.map((season) => {
                return (
            ...
            ? props.data.map((club) => {
                return (
                  ...
                  >
                    ...
                    <td key={`${club.name}_last5`}>
                      {club.last_five_matches.map((m, i) => {
                        return (
                          <span
                            key={`${m}_${i}`}
                            style={{
